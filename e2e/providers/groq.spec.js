/**
 * Tests E2E — Provider groq (Sprint B)
 *
 * Groq = inference ultra-rapide (LPU). Couvre :
 *   1. setProvider + chargement config
 *   2. Chat completion réel
 *   3. Streaming SSE OpenAI-compat
 *   4. Format markdown
 *
 * @slow (skip gracieux si GROQ_API_KEY absente)
 */
...
test.describe('Provider groq @slow', () => {
  test.beforeEach(async ({ page }) => {
    skipIfNoKey(test, 'groq');
    await page.goto('/');
  ...
  test('1 - setProvider(groq) charge la config par défaut', async ({ page }) => {
    await setupProvider(page, 'groq');
    ...
    expect(provider.model).toBe('groq/compound');
  ...
  test('2 - chat completion renvoie une réponse du modèle', async ({ page }) => {
    ...
    if (!result.success) {
      test.skip(true, `Groq non accessible (${result.error?.slice(0, 80)}…) — clé ou réseau`);
      return;
    }
  ...
  test('4 - format markdown parsé (gras + code inline)', async ({ page }) => {
    ...
    if (!result.success) {
      test.skip(true, `Groq non accessible (${result.error?.slice(0, 80)}…)`);
      return;
    }
